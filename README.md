...
flowchart TD
    ...
    F --> G["构建 ItemPool"]
    G --> H["枚举搜索<br>分支定界 + 附魔价值评分"]

    H --> I{"每步评估"}
    I --> K["记录候选解<br>附魔价值优先 · 成本次之"]
    I --> L["尝试所有合并配对"]
    L --> M{"mergedEV > bestEV<br>或成本更低?"}
    M -- 是 --> N["递归搜索子树"]
    M -- 否 --> O["剪枝跳过"]
    N --> I
...
- **上界剪枝** — 预计算最大可能附魔价值（含同级合并升级），提前排除无望分支
- **对称性剪枝** — 完全相同的附魔书只尝试一个合并方向，搜索量减半
- **双指标评分** — 先最大化附魔价值，再最小化成本，避免跳过有效升级
- **类型约束** — 自动跳过不兼容的物品合并（剑+镐等）
- **超时保护** — 大量物品时自动限时返回当前最优解
